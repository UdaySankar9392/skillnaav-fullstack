...
const Sidebar = () => {
  ...
  const { handleSelectTab } = useTabContext();// State for mobile menu toggle
  ...
  const handleTabClick = (tab) => {
    if (tab === "logout") {
      ...
      setSelectedTab(tab);
      handleSelectTab(tab);
      setIsOpen(false); // Close the hamburger menu when a tab is selected
    }
  };
  ...
  return (
    <div>
      ...
      {/* Sidebar */}
      <div
        className={`lg:w-64 w-full h-screen bg-white flex flex-col justify-between pl-6 pr-6 font-poppins shadow-lg sticky top-0 overflow-y-auto scrollbar-hide ${
          isOpen ? "block" : "hidden" // Show sidebar on mobile if isOpen is true
        } lg:block`} // Keep the sidebar visible on large screens
      >
        {/* Logo Section */}
